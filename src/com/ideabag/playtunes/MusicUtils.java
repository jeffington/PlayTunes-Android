	...
	public static String getCommandText(Context context, int command) {
		...
		switch (cmd) {
			...
			case COMMAND_ARTIST_CODE: //MediaStore.Audio.Artists.ARTIST, MediaStore.Audio.Artists._ID 
				c = context.getContentResolver().query(MediaStore.Audio.Artists.EXTERNAL_CONTENT_URI, artistsSelection, MediaStore.Audio.Artists._ID +"="+id, null, MediaStore.Audio.Artists.ARTIST);
				if (!c.moveToFirst())
					str = "";
				else
			...
			case COMMAND_PLAYLIST_CODE:
    			c = context.getContentResolver().query(MediaStore.Audio.Playlists.EXTERNAL_CONTENT_URI, allPlaylistsSelection, MediaStore.Audio.Playlists._ID+"="+id, null, MediaStore.Audio.Playlists.NAME);
				if (!c.moveToFirst())
					str = "";
				else
			...
			case COMMAND_GENRE_CODE:
				c = context.getContentResolver().query(MediaStore.Audio.Genres.EXTERNAL_CONTENT_URI, allGenresSelection, MediaStore.Audio.Genres._ID+"="+id, null, MediaStore.Audio.Genres.NAME);
    			if (!c.moveToFirst())
					str = "";
				else
					str = c.getString(c.getColumnIndexOrThrow(MediaStore.Audio.Playlists.NAME));
